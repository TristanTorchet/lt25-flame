...
def main():
    ...
    def create_padded_labels_scatter(labels, target_lengths, batch_size, pad_token_id=0):
        """
        Ultra-efficient vectorized approach using scatter operations
        Claude generated
        """
        max_length = target_lengths.max().item()
        
        # Initialize output tensor
        padded_labels = torch.full((batch_size, max_length), pad_token_id, 
                                  dtype=labels.dtype)
        
        # Create batch indices for each element in labels
        batch_indices = torch.repeat_interleave(torch.arange(batch_size), target_lengths)
        
        # Create position indices within each sequence
        position_indices = torch.cat([torch.arange(length) for length in target_lengths])
        
        # Scatter values into 2D tensor
        padded_labels[batch_indices, position_indices] = labels
    ...
    def train_epoch(model, dataloader, optimizer, scheduler, device, epoch, log_interval, logger, max_grad_norm=1.0, use_wandb=False, idx_to_char=None, print_samples=False, gradient_accumulation_steps=1):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            #check if inputs and labels are empty

            input_lengths = batch['feature_lengths'].to(dtype=torch.int)
            target_lengths = batch['target_lengths'].to(dtype=torch.int)
            ...
            loss = torch.sum(loss) / gradient_accumulation_steps
            loss.backward()
